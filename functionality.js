class Hero {
    ...
    attacked(damage) {
      if (this.canFly) {
        ...
        let chance = Math.random(); // chance rep o var pentru a adauga dinamicitate jocului; poate sa aiba valori intr 0 si 1
        if (chance > 0.5) {
          // sanse foarte mari
          console.log(this.name + ' ' + ' flwe away.'); // eroul zboara si evita damage-ul (damamge = 0)
          damage = 0;
        }
      }
      if (this.shield) {
        damage *= 0.8; // damage-ul scade cu 0.2
        console.log(this.name + ' defends whith a schield.');
      }
